...
class PlayerController:
    ...
    def __init__(self, startpos: Vector2):
        ...
        self.max_jump_time = 1.2
        self.terminal_vel = 1500

        time_to_apex = self.max_jump_time / 2
        self.gravity = 2 * self.max_jump_height / (time_to_apex**2)
        self.jump_force = 2 * self.max_jump_height / time_to_apex
    ...
    def update(self):
        if (self.is_grounded):
            ...
            self.surface_vel = Vector2(Vector2.dot(self.move_dir, self.vel), Vector2.dot(self.jump_dir, self.vel))
    ...
    def apply_horizontal_accel(self):
        if (self.input == 0):
            if (self.surface_vel.x == 0): return
        ...
        self.surface_vel.x = clamp(self.surface_vel.x, -self.speed, self.speed)
    ...
    def check_jump(self):
        ...
        if (self.is_grounded and self.jump_pressed and self.can_jump):
            self.can_jump = False
            self.surface_vel.y = self.jump_force * max((Vector2.dot(self.surface_normal, Vector2(0, -1)) + 1) / 2, 0.1)
    ...
    def handle_collision(self):
        ...
        info = CollisionHandler.circle_circle(self.coll, PlatformManager.obj)
        if (info is None):
            return
